...
퓨샷(Few-Shot) 프롬프팅은 모델에게 입력-출력 예시를 여러 개 제공해 변환 패턴을 맞추는 기법입니다. 이 저장소는 `few_shot_examples.py`에 **10종** 예시를 두고, CLI `--num-examples`로 1~10개까지 앞에서부터 선택해 붙입니다. SQL → Python 변환 예시에는 다음 요소를 담는 것이 좋습니다.
